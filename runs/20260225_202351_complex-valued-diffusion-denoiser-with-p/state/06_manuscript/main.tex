...
\usepackage{multirow}
\usepackage{booktabs}
% Reduce white space around floats, captions, and bibliography
...
\setlength{\dblfloatsep}{1pt plus 1pt minus 1pt}
...
\author{\IEEEauthorblockN{AI Agents\thanks{Submitted to ICMLCN 2026 Experimental Track (Telecom AI Researcher). Developed by Sen Yan, Yuzhi Yang, and Merouane Debbah (Digital Future Institute, Khalifa University, UAE), and Samson Lasaulce (CNRS \& Universit\'{e} de Lorraine, CRAN, France).}}
...
\end{equation}
This representation directly concatenates the noisy channel, LS estimate, and pilot mask as input channels, avoiding iterative pilot replacement during inference (Fig.~\ref{fig:system}).
...
Figure~\ref{fig:ber} shows the BER performance after zero-forcing equalization. All methods exhibit similar trends, with MMSE achieving the lowest error rates as expected. The diffusion-based methods (ComplexUNet and RealUNet) perform comparably to the DNN baseline, indicating that while they do not significantly improve channel estimation accuracy, they maintain acceptable symbol detection performance.
...
\textbf{SNR-Dependent Gains}: At high SNR (25 dB), ComplexUNet achieves $-19.79$ dB NMSE, outperforming the DNN baseline by 1.2 dB, while the DNN performs comparably at low-to-medium SNR—indicating the diffusion prior is most beneficial when channel structure is recoverable.

\textbf{Phase Rotation Gate}: The marginal average difference between ComplexUNet ($-13.19$ dB) and RealUNet ($-13.43$ dB) indicates the Rz gate does not consistently improve NMSE, but provides a principled complex-valued output avoiding the circle constraint of polar parameterizations.
...
These results establish that conditioning design is the critical enabling factor for diffusion-based OFDM estimation. Future work should explore attention-based conditioning, high-mobility channels, and MIMO extensions.
